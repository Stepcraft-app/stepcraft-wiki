---
title: Pumpkin
lastVerified: 2026-05-10
---

<div className="flex items-start gap-6 mb-8">
  <div className="flex-shrink-0">
    <img src="/assets/resources/farming/pumpkin.png" alt="Pumpkin" className="w-24 h-24 border rounded-lg" />
  </div>
  <div className="flex-1">
    <div className="space-y-2">
      <p className="text-lg text-muted-foreground">A seasonal resource used for crafting and trading in Stepcraft.</p>
      <div className="flex flex-wrap gap-4 text-sm">
        <span className="px-2 py-1 bg-blue-100 dark:bg-blue-900 rounded-md">
          **Category:** Farming
        </span>
        
        <span className="px-2 py-1 bg-green-100 dark:bg-green-900 rounded-md">
          **Price:** 31 gold
        </span>
      </div>
    </div>
  </div>
</div>

## Found at

- [Blooming Glade](/docs/map/individual/blooming_glade)

## Obtained by Farming

**Level Requirement:** 30 | **XP Gained:** 50 | **Steps:** 100 | **Tool Required:** None



## Used in Recipes

This resource is used in the following crafting recipes:

| Recipe | Skill | Level | XP | Produces |
|--------|-------|-------|----|---------:|
