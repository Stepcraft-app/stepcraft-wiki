...
// Import item data without requiring assets
...
function extractIconPath(iconRequire: any): string {
...
async function generateAllItemPages() {
  ...
  try {
    ...
    // const items = Object.values(allItems); // Disabled - using generate-all-item-pages.ts instead
    const items: any[] = [];
